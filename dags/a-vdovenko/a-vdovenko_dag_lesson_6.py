...
dag = DAG("a-vdovenko_load_cbr",
          schedule_interval='@daily',
          default_args=DEFAULT_ARGS,
          max_active_runs=1,
          tags=['a-vdovenko_lesson_6']
          )

# TODO: Не константа, а {{ format_ds... }}
# TODO: удалять файл anton_cbr.xml, если он уже есть
load_cbr_xml_script = '''
curl https://www.cbr.ru/scripts/XML_daily.asp?date_req=01/11/2021 | iconv -f Windows-1251 -t UTF-8 > /tmp/anton_cbr.xml
'''
...
def export_xml_to_csv_func():
    ...
    tree = ET.parse('/tmp/anton_cbr.xml', parser=parser)
    root = tree.getroot()

    with open('/tmp/anton_cbr.csv', 'w') as csv_file:
...
def load_csv_to_gp_func():
    ...
    pg_hook.copy_expert("COPY anton_cbr FROM STDIN DELIMITER ','", '/tmp/anton_cbr.csv')
