...
def xml_to_csv_func():
    parser = ET.XMLParser(
        encoding="UTF-8")  # Создаем Parser формата UTF-8, натравливаем его на нужный файл ('/tmp/dina_cbr.xml', parser=parser)
    tree = ET.parse('/tmp/dina_cbr.xml', parser=parser)
    root = tree.getroot()  # Корень нашего файла

    with open('/tmp/dina_cbr.csv',
...
def load_csv_to_greenplum_func():
    ...
    pg_hook.copy_expert("COPY y_bainazar_cbr FROM STDIN DELIMITER ','", '/tmp/dina_cbr.csv')
