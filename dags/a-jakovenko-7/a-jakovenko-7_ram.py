...
from a_jakovenko_7_plugins.mn_ram_top_location_operator import MnRamTopLocationOperator
...
def load_json_to_greenplum_func(ti):
        ...
        with conn.cursor() as cur:
            cur.execute("""
                CREATE TABLE IF NOT EXISTS public.a_jakovenko_7_ram_location (
                     id int, name varchar, type varchar, dimension varchar, resident_cnt int);
                TRUNCATE TABLE public.a_jakovenko_7_ram_location;
                COMMIT;"""
                )
            for i in range(3): # топ 3 локации
                query = f"""
                    INSERT INTO public.a_jakovenko_7_ram_location (id, name, type, dimension, resident_cnt)
...
with DAG("a-jakovenko-7_ram",
         schedule_interval='@daily',
         default_args=DEFAULT_ARGS,
         tags=['a-jakovenko-7_ram'],
         max_active_runs=1
         ) as dag:
