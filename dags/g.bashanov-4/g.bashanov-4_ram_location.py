...
from g_bashanov_4_plugins.mn_ram_top_location_operator import MnRamTopLocationOperator
...
def load_json_to_greenplum_func(ti):
        ...
        with conn.cursor() as cur:
            cur.execute("""
                CREATE TABLE IF NOT EXISTS public.g_bashanov_4_ram_location (
                     id int, name text, type text, dimension text, resident_cnt int);
                TRUNCATE TABLE public.g_bashanov_4_ram_location;"""
                )
            for i in range(3): # топ 3 локации
                query = f"""
                INSERT INTO public.g_bashanov_4_ram_location (id, name, type, dimension, resident_cnt)
...
with DAG("g_bashanov_4_morty", schedule_interval='@daily',
         default_args=DEFAULT_ARGS, tags=['g.bashanov-4'], max_active_runs=1) as dag:
