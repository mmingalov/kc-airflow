...
class DubovikOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        for page in range(self.get_pages_count(url.format(pg='1'))):
            ...
            if r.status_code == 200:
                logging.info('HTTP Status: 200 OK')
                locations_df_page = pd.DataFrame(self.get_location(r.json().get('results')),
                                                 columns=df_columns).sort_values(by=['resident_cnt'],
        ...
        locations = locations.sort_values(by=['resident_cnt'], ascending=False) \
                             .head(self.pages_count)
